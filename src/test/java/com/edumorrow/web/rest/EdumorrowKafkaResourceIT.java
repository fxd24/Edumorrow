...
import com.edumorrow.config.KafkaProperties;
...
class EdumorrowKafkaResourceIT {
    ...
    void setup() {
        ...
        EdumorrowKafkaResource kafkaResource = new EdumorrowKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/edumorrow-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/edumorrow-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
